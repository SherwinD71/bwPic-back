const getDB = require("../../db");

const votePhoto = async (req, res, next) => {
  let connection;
  try {
    connection = await getDB();

    // codigo ...
    const { id_photos } = req.body;
    const { vote } = true;
    const { id_users } = req.body;

    //añado el comentario a la base de datos

    await connection.query(
      `
      INSERT INTO likes(vote, created_at, id_photos, id_users)
      VALUES (?,?,?,?)
    `,
      [vote, new Date(), id_photos, id_users]
    );

    res.send({
      status: "ok",
      message: "votePhoto",
      data: "data",
    });
  } catch (error) {
    next(error);
  } finally {
    if (connection) connection.release();
  }

  // let connection;
  // try {
  //   connection = await getDB();
  //   const { id } = req.params;
  //   const { vote } = req.body;

  //   if (vote < 1 || vote > 1) {
  //     const error = new Error("El voto tiene que ser 1");
  //     error.httpStatus = 400;
  //     throw error;
  //   }

  //   // no puede votar el usuario que la creó
  //   const [current] = await connection.query(
  //     `
  //     SELECT user_id
  //     FROM photos
  //     WHERE id_users=?
  //   `,
  //     [id]
  //   );

  //   if (current[0].user_id === req.userAuth.id) {
  //     const error = new Error("No puedes votar tu propia foto");
  //     error.httpStatus = 403;
  //     throw error;
  //   }

  //   // un usuario puede votar solo una vez
  //   const [existingVote] = await connection.query(
  //     `
  //     SELECT id_likes
  //     FROM likes
  //     WHERE id_users=? AND id_photos=?
  //   `,
  //     [req.userAuth.id, id]
  //   );

  //   if (existingVote.length > 0) {
  //     const error = new Error("Ya votaste esta foto");
  //     error.httpStatus = 403;
  //     throw error;
  //   }

  //   const now = new Date();

  //   // añado el voto a la tabla
  //   await connection.query(
  //     `
  //       INSERT INTO likes (created_at, vote, id_photod, id_users)
  //       VALUES (?,?,?,?)
  //   `,
  //     [now, vote, id, req.userAuth.id]
  //   );

  //   // // saco la nueva media de los votos
  //   // const [newVotes] = await connection.query(
  //   //   `
  //   //     SELECT AVG(vote) AS media_votes
  //   //     FROM entries_votes
  //   //     WHERE entry_id = ?
  //   //     GROUP BY entry_id
  //   // `,
  //   //   [id]
  //   // );

  //   // console.log(newVotes);

  //   res.send({
  //     status: "ok",
  //     data: {
  //       // votes: newVotes[0].media_votes,
  //     },
  //   });
  // } catch (error) {
  //   next(error);
  // } finally {
  //   if (connection) connection.release();
  // }
};

module.exports = votePhoto;
